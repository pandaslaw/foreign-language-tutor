        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"✅ {reminder_type}", callback_data=f"reminder_{reminder_type}_on"
                )
            ]
        )

    keyboard.append([InlineKeyboardButton("Done ✨", callback_data="reminders_done")])
...
async def finish_onboarding(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        scheduler: LearningScheduler = context.application.scheduler
        default_times = {"morning": "09:00", "afternoon": "15:00", "evening": "22:00"}
