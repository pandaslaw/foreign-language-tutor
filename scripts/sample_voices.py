...
def preview_voice(voice: Voice):
    ...
    for text in test_texts:
        ...
        try:
            # Generate audio
            audio = generate(
                text=text, voice=voice.name, model="eleven_multilingual_v2"
            )
...
def main():
    ...
    for voice in all_voices:
        ...
        if any(
            word in description
            for word in ["warm", "soft", "gentle", "natural", "female", "professional"]
        ):
            # Exclude voices that don't match Leyla's character
            if not any(
                word in description
                for word in ["robotic", "artificial", "child", "old"]
            ):
    ...
    female_voices.sort(
        key=lambda v: sum(kw in get_voice_description(v) for kw in positive_keywords),
        reverse=True,
    )
